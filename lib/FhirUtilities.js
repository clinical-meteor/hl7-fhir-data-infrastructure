...
export const FhirUtilities = {
  ...
  consentIntoAccessControl(consentRecord){
    ...
    // should we base the action on the category coding?  
    if(get(consentRecord, 'provision[0].provision[0].actor[0].role.coding[0].display')){
      result.role = get(consentRecord, 'provision[0].provision[0].actor[0].role.coding[0].display');
    }

    if(get(consentRecord, 'provision[0].provision[0].securityLevel[0].coding[0].display')){
      result.condition = {
        Fn: "EQUALS", 
        args: { 
          confidentiality: uniq(["normal", get(consentRecord, 'provision[0].provision[0].securityLevel[0].coding[0].display')])
        }
      }
    }
