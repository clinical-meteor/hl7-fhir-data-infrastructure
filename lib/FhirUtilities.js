...
export const FhirUtilities = {
  ...
  consentIntoAccessControl(consentRecord){
    ...
    if(get(consentRecord, 'provision.provision[0].securityLabel[0].display')){
      result.qualifier = "EQUALS"
      result.args = "securityLabel"
      result.securityLabel = get(consentRecord, 'provision.provision[0].securityLabel[0].display');
      result.condition = {
        Fn: "EQUALS", 
        args: { 
          securityLabel: get(consentRecord, 'provision.provision[0].securityLabel[0].display')
        }
      }
    }
