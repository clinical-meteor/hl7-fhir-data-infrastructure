...
export function lookupReferenceName(referenceString){
  ...
  if(typeof window[FhirUtilities.pluralizeResourceName(resourceBase)] === "object"){
    ...
    let lookupRecord = window[FhirUtilities.pluralizeResourceName(resourceBase)].findOne({id: FhirUtilities.pluckReferenceId(referenceString)});

    // console.log('lookupRecord', lookupRecord)
    if(typeof lookupRecord.name === "string"){
      result = get(lookupRecord, 'name');
    } else if (Array.isArray(lookupRecord.name)){
      result = FhirUtilities.assembleName(lookupRecord.name[0])
    } else {
