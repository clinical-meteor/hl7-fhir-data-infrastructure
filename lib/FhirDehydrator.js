...
export function flattenEndpoint(endpoint, internalDateFormat){
    ...
    result.connectionType = get(endpoint, 'connectionType[0].display');
...
export function flattenHealthcareService(service, internalDateFormat){
    ...
    result.category = get(service, 'category[0].text', '');
    result.type = get(service, 'type[0].text', '');
    result.specialty = get(service, 'specialty[0].text', '');
...
export function flattenInsurancePlan(plan, internalDateFormat){
    ...
    result.identifier = get(plan, 'identifier[0].value');
    result.status = get(plan, 'status');
    result.type = get(plan, 'type[0].text');
    result.name = get(plan, 'name');
    result.alias = get(plan, 'alias');

    result.ownedBy = get(plan, 'ownedBy.display');
    result.administeredBy = get(plan, 'administeredBy.display');
...
export function flattenOrganizationAffiliation(organization, internalDateFormat){
    let result = {
      ...
      periodStart: '',
      periodEnd: '',
      usageRestrictionDisplay: '',
      usageRestrictionReference: '',
    };
    ...
    result.usageRestrictionDisplay = get(organization, 'usage-restriction[0].display', '');
    result.usageRestrictionReference = get(organization, 'usage-restriction[0].reference', '');

    result.organization = get(organization, 'organization.display', '');
    result.participatingOrganization = get(organization, 'participatingOrganization.display', '');
    result.network = get(organization, 'network[0].display', '');
    result.location = get(organization, 'location[0].display', '');
    result.healthcareService = get(organization, 'healthcareService[0].display', '');

    result.code = get(organization, 'code[0].text', '');
    result.specialty = get(organization, 'specialty[0].text', '');
...
export function flattenPractitionerRole(role, internalDateFormat){
    ...
    result.identifier = get(role, 'identifier[0].value');
    ...
    result.practitioner = get(role, 'practitioner.display', '');
    result.organization = get(role, 'organization.display', '');
    result.location = get(role, 'location[0].display', '');
    result.healthcareService = get(role, 'healthcareService[0].display', '');

    result.code = get(role, 'code[0].text', '');
    result.specialty = get(role, 'specialty[0].text', '');
...
export function flattenRestriction(restriction, internalDateFormat){
    ...
    result.resourceType = get(document, 'resourceType', "Restriction");
...
export const FhirDehydrator = {
  ...
  dehydrateOrganizationAffiliation: flattenOrganizationAffiliation,
  dehydratePatient: flattenPatient,
  dehydratePractitioner: flattenPractitioner,
  dehydratePractitionerRole: flattenPractitionerRole,
